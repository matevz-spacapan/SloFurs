...
class InvoiceModel{
	...
	*/
	...
	public function getInvoice($id){
		//TODO check if invoice is for this account
		$id=strip_tags($id);
		$sql='SELECT registration.id AS id, event_start AS due, ticket, room_confirmed AS rconfirmed, room.type AS rtype, room.price AS rprice, event.name AS ename, regular_price AS regular, sponsor_price AS sponsor, super_price AS super, fname, lname, address, address2, post, city, country FROM registration INNER JOIN event ON event.id=registration.event_id INNER JOIN room ON registration.room_id=room.id INNER JOIN account ON account.id=registration.acc_id WHERE registration.id=:id';
	...
	public function download($id){
		...
		$mpdf=new \Mpdf\Mpdf([
			'fontDir'=>array_merge($fontDirs,[
				'public/vendor/mpdf/mpdf/ttfonts',
			]),
			'fontdata'=>$fontData+[
				'muli'=>[
					'R' => 'Muli-Regular.ttf',
					'I' => 'Muli-RegularItalic.ttf',
					'B' => 'Muli-Bold.ttf',
				]
			],
			'default_font'=>'muli'
		...
		<div>
		  ...
		  <div class='w3-row w3-padding-32 w3-padding-large'>
				<table class='w3-table'>
					<tr>
						<td>
						For:<br>
							{$invoice->fname} {$invoice->lname}<br>
							{$invoice->address}<br>
							{$invoice->post} {$invoice->city}<br>
							{$invoice->country}
						</td>
						<td>
						Issued by:<br>
		  ...
		  <div class='w3-row w3-padding-large'>
		    <table class='w3-table w3-striped w3-bordered'>
		      ...
		      <tr>
		        <td>{$invoice->ename} Attendance ({$invoice->ticket})</td>
		        <td>";
		          switch($invoice->ticket){
		            case 'regular':
		              $text.="{$invoice->regular}€";
		              $total+=$invoice->regular;
		              break;
		            case 'sponsor':
		              $text.="{$invoice->sponsor}€";
		      ...
		      if($invoice->rprice!=null&&$invoice->rconfirmed==1){
		        $total+=$invoice->rprice;
						$paid=0;
		        $difference=$total-$paid;
		        $text.="<tr>
		            <td>Accomodation ({$invoice->rtype})</td>
		            <td>{$invoice->rprice}€</td>
		        </tr>";
		      }
		    $text.="</table>
		  </div>
		  <div class='w3-row w3-padding-large'>
		    <div class='w3-right' style='width:300px;'>
		      <table class='w3-table w3-bordered'>
		        ...
		        <tr>
		          <td>Due date</td>
		          <td>{$this->convertViewable($invoice->due, 1)}</td>
		        </tr>
		      </table>
		    </div>
		  </div>
		...
		$mpdf->WriteHTML($text);
		$mpdf->SetTitle('Invoice no. '.$id);
		$mpdf->Output('Invoice_'.$id.'.pdf', \Mpdf\Output\Destination::DOWNLOAD);
	}
}
